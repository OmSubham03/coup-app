...
function renderNQAsking(tableArea, actionArea, isGiver, me) {
  ...
  for (const q of nqState.questions) {
    ...
    qaHtml += `<div class="nq-qa-item${isGuessClass}${correctClass}">
      <div class="nq-q"><span class="nq-q-badge">Q${q.turn}</span> <strong>${esc(q.askerName)}</strong>: ${esc(q.question)}</div>
  ...
  if (nqState.waitingForAnswer) {
    ...
    actionArea.innerHTML = `
      <div class="nq-ask-form">
        <p class="nq-prompt">Your turn! Ask a question or make a guess.</p>
        ...
        <div class="nq-ask-buttons">
          <button class="btn nq-btn-ask" onclick="nqAskQuestion()">Ask</button>
          <button class="btn nq-btn-guess-toggle" onclick="nqShowGuessForm()">Make a Guess</button>
        </div>
        <div id="nq-guess-form" style="display:none">
    ...
    actionArea.innerHTML = `<div class="nq-wait-msg">${esc(currentAsker?.name || '...')}'s turn to ask...</div>`;
  }
...
function nqAskQuestion() {
  ...
  if (!question) return;
  send('nq-ask', { question, isGuess: false, guessWord: '' });
}
function nqShowGuessForm() {
  const form = document.getElementById('nq-guess-form');
  if (form) form.style.display = form.style.display === 'none' ? '' : 'none';
}
function nqMakeGuess() {
  const questionInput = document.getElementById('nq-question-input');
  const guessInput = document.getElementById('nq-guess-input');
  const guess = guessInput.value.trim();
  if (!guess) return;
  send('nq-ask', { question: `I think it's "${guess}"`, isGuess: true, guessWord: guess });
}
function nqFinalGuess() {
  const input = document.getElementById('nq-final-guess-input');
  const guess = input.value.trim();
  if (!guess) return;
  send('nq-final-guess', { guess });
}
